...
void drawArm() {
	...
	for (int i = 0; i < drawingPointsCount; i++) {
		glVertex3d(drawing[0][i], drawing[1][i], drawing[2][i]);}	
	glEnd();
	...
	glVertex3d(n1x, n1y,  0);	glVertex3d(n1x, n1y,    h);		
	glEnd();
...
void loop(int) {
	...
	if (pause == 0) {
		...
		if (followPath == 1) {
			angFind(path[0][pathPt], path[1][pathPt], path[2][pathPt]);		
			pathPt++;
